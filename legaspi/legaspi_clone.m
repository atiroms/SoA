...
Åf*5 Plot and store CCE as function of causal prior strength
...
arrPXil,arrPrcShftA(2,:),;r-', arrPXil,arrPrcShft0(2,:),'r?-',Å·e.
...
lgnd = legend('Voluntary action',' and tone', ÅeInvoluntary action',' and tone','Sham',' and tone', 'Location', 'northwest', 'Orientation','vertical');
lgnd.FontSize = 18;
...
lgnd = legend('Action', ' and low uncertainty toneÅf, 'Action', ' and intermediate uncertainty tone, 'Action', ' and high uncertainty toneÅf, 'Location', 'northwest');
lgnd.FontSize = 18;
set(lgnd.BoxFace, 'ColorType', 'truecoloralpha', 'ColorData', uint8(255*[1; 1; 1; @.8]));
...
elseif flag == 2
errorbar(mean(reshape(arrAxes(2,:),SsizeBin, []),1), mean(reshape(arrBehavior(2,:),sizeBin,[]),1), std(reshape(arrBehavior(2,:),sizeBin,[]),1I},', 'ÅeLineWidth', LineWidth,'MarkerSize', /
markerSize) ;
...
errorbar(mean( reshape(arrAxes(2,:),SizeBin, []),1), mean(reshape(arrBehavior(2,:),sizeBin,[]),1), std(reshape(arrBehavior(2,:),sizeBin,[]),1fplor', [@ 140/255 255/255] Åh
'LineStyle','none', 'Marker', '.', 'LineWidth', LineWidth, 'MarkerSize',markerSize) ;
errorbar(mean( reshape(arrAxes(3,:),sizeBin, []),1), mean(reshape(arrBehavior(3,:),sizeBin,[]),1), std(reshape(arrBehavior(3,:),sizeBin,[]),2XÅíplor', [0 240/255 255/255] Åh@
'LineStyle','none', 'Marker', '.', 'LineWidth', LineWidth,'MarkerSize',markerSize) ;
end
lgnd = legend('Low uncertainty', ÅeIntermediate uncertaintyÅf, 'High uncertainty', 'Location', 'northwest');
set(lgnd.BoxFace, 'ColorType', Åetruecoloralpha', 'ColorData', uint8(255*[1; 1; 1; 0.8]));
set(gca, 'Box', 'on');
hold off
...
if experiment == 1
if behavior == 1
plot( arrPXil,arrBehavior(1,:)};b', arrPXil,arrBehavior(2,:),'r', arrPXil,arrBehavior(3,:),'k', ÅeLinewidth', linewidth);
%{
hold all
...
%}
elseif behavior == 3
plot( arrPXil,arrBehavior(2,:),r', arrPXil,arrBehavior(1,:),'b', arrPXil,arrBehavior(3,:),'k', ÅeLinewidth', linewidth);
elseif behavior == 0
...
if experiment == 1
if behavior == 1
lgnd = legend(ÅeVoluntary condition','Involuntary condition','Sham condition', 'Location', 'northwest');
elseif behavior ==3
lgnd = legend('Voluntary condition','Involuntary condition','Sham condition', 'Location', 'northwest');
else
...
%set(lgnd, 'Color', 'none');
set(gca,'color', 'white');
set(lgnd.BoxFace, 'ColorType', Åetruecoloralpha', 'ColorData', uint8(255*(1; 1; 1; @.8]));
% If you want to bold
